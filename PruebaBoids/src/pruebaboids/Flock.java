 ...
 *
 * A number of rules are applied to ensure the flock exhibits the desired
 * characteristics.
 *
...
public class Flock {
    ...
    public Flock() {
        ...
        //Colours used in testing to help illustrate individual movement. 
        boids.add(new TestBoid(400, 100, "Red", new ImageIcon("images/nuevoRaton.png")));
        boids.add(new TestBoid(700, 100, "Green", new ImageIcon("images/boid-green.png")));
        boids.add(new TestBoid(250, 30, "Blue", new ImageIcon("images/boid-blue.png")));
    ...
    /**
     * Updates the position coordinates of each boid in the flock. By applying
     * the rules that govern the behaviour of the group to each one in turn.
     */
...
//        for (Boid cBoid : boids) //tener una lista tambien de depredadores y de comida
     ...
     *
     * Returns a vector representing the boids perceived centre of the group
     * (Average position of all boids not including the boid itself). A movement
     * factor is applied to move the boid a percentage of the way towards the
     * center.
     *
     * @param	cBoid	The boid which the rule is applied to
     * @return Vector	Grid position moving the boid towards the center of the
     * group
     *
     */
     ...
     *
     * Checks to see if the boid is within a specified distance of other boids
     * by comparing the position coordinates of each. If the flock mate is
     * inside the minimum distance, the vector is updated to move the boid
     * further away by half of the current distance between the two.
     *
     * @param	cBoid	The boid which the rule is applied to
     * @return	Vector	Grid coordinates of a position that would take the boid
     * away from each flock mate that is too close.
     *
     */
     ...
     *
     * Returns a vector representing the boids perceived average velocity of the
     * flock, not including the boid itself.
     *
     * @param	cBoid	The boid which the rule is applied to
     ...
     *
     * Draws the boids towards points of attraction. Acts as a goal for boids
     * helping to provide more predictable group behaviour.
     *
     * @param cBoid	The boid which the rule is applied to
     * @return	Vector	A grid position moving the boid towards points of
     * attraction
     */
    ...
    /**
     * Returns a list of <Code>ImageView</Code> objects for every boid
     *
     * @return Visual representation of all birds
     */
    public List<ImageIcon> getBoidsRepresentation() {
        List<ImageIcon> images = new ArrayList<ImageIcon>();
        for (Boid aBoid : boids) {
            images.add(aBoid.getImagenBoid());
        }
        return images;
    }
